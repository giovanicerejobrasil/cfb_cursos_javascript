export default class Login {
    ...
    static verifyLogin = () => {
        ...
        const endpoint = `${this.config.endpoint}?registration=${registration}&password=${password}`;
        fetch(endpoint)
            .then(res => res.json())
            .then(res => {
                console.log(res);
                if (res) {
                    ...
                    sessionStorage.setItem('registrationLogged', registration);
                    sessionStorage.setItem('nameLogged', res.name);
                    sessionStorage.setItem('accessLogged', res.access);
                    this.callbackOK();
                    this.closeLogin();
